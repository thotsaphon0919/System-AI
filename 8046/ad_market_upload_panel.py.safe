...
def setup_ad_market_upload_panel(app) -> None:
    @app.middleware("http")
    async def ad_market_upload_panel_middleware(request: Request, call_next):
        path = request.url.path

        # เปิดรูปที่อัปโหลด
        if path.startswith("/tower/ad-market-media/"):
        ...
        # รับรูปจากแผงอัปโหลด
        if path == "/tower/ad-market-upload" and request.method == "POST":
            form = await request.form()

            image = form.get("ad_market_image")
            config = load_config()
        ...
        if path != "/tower/ad-market" or request.method != "GET":
            return response
        ...
        if image_name:
            hero_html = f"""
            <section class="ad-market-top-image">
              <img
                src="/tower/ad-market-media/{image_name}"
        ...
        <style>
          ...
          .ad-market-upload-panel.open {{
            right: 0;
          }}

          .ad-market-upload-panel h2 {{
            margin-top: 12px;
            color: #c985ff;
          }}

          .ad-market-upload-panel label {{
            display: block;
            margin: 16px 0 7px;
          }}

          .ad-market-upload-panel input {{
            width: 100%;
            padding: 13px;
          ...
          .ad-market-upload-panel button[type="submit"] {{
            width: 100%;
            margin-top: 20px;
            padding: 14px;
        ...
        <aside
          id="adMarketUploadPanel"
          class="ad-market-upload-panel"
        >
          <button
            class="infini-close"
            type="button"
          ...
          <h2>จัดการรูป AD Market</h2>
          <p>อัปโหลดภาพ และกำหนดลิงก์ของช่องในภาพ</p>

          <form
            method="post"
            action="/tower/ad-market-upload"
            enctype="multipart/form-data"
          >
            <label>อัปโหลดรูป AD Market</label>
            <input
              type="file"
              name="ad_market_image"
              accept="image/*"
              onchange="previewAdMarketImage(event)"
            >
        ...
        <script>
          function toggleInfiniUploadPanel() {{
            document
              .getElementById("adMarketUploadPanel")
              .classList.toggle("open");
          }}

          function previewAdMarketImage(event) {{
        ...
        if hero_html:
            ...
            elif '<div class="wrap">' in page_html:
                page_html = page_html.replace(
                    '<div class="wrap">',
