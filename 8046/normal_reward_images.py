...
UPLOAD_DIR = BASE_DIR / "uploads" / "normal_reward_media"
INDEX_FILE = DATA_DIR / "normal_reward_media.json"
...
def _load_index() -> dict[str, Any]:
    ...
    try:
        ...
        return data if isinstance(data.get("rewards"), dict) else {"rewards": {}}
...
def _media_block(reward_id: str, detail: bool = False) -> str:
    safe_id = _clean_reward_id(reward_id)
    info = _load_index()["rewards"].get(safe_id)

    if info and _kind(info) == "video":
        options = "controls" if detail else "autoplay muted loop"
        _stored_name = info.get("stored_name", "")
        _size = info.get("size", "")
        media = (
            f'<video src="/normal-reward-media/{safe_id}?v={_stored_name}-{_size}" '
            f'playsinline preload="metadata" {options} '
            'style="width:100%;height:100%;object-fit:cover;'
            'display:block;background:#000"></video>'
        )
    elif info:
        _stored_name2 = info.get("stored_name", "")
        _size2 = info.get("size", "")
        media = (
            f'<img src="/normal-reward-media/{safe_id}?v={_stored_name2}-{_size2}" '
            f'alt="สื่อรางวัล {safe_id}" loading="lazy" '
            'style="width:100%;height:100%;object-fit:cover;'
    ...
    return (
        f'<a href="/normal-reward-upload/{safe_id}" '
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if is_list:
            for reward_id in REWARD_ORDER:
                html = html.replace("รูปของรางวัล", _media_block(reward_id), 1)
        else:
            reward_id = path.rsplit("/", 1)[-1]
            html = html.replace(
                "รูปและรายละเอียดของรางวัล",
                _media_block(reward_id, detail=True),
                1,
            )
...
def setup_normal_reward_images(app: FastAPI) -> None:
    app.add_middleware(NormalRewardMediaMiddleware)

    @app.get("/normal-reward-upload/{reward_id}", response_class=HTMLResponse)
    async def upload_page(reward_id: str, request: Request) -> HTMLResponse:
        safe_id = _clean_reward_id(reward_id)
...
<div class="wrap">
  <div class="card">
    ...
    <div class="pad">
      ...
      <form method="post" action="/normal-reward-upload/{safe_id}" enctype="multipart/form-data">
    ...
    @app.post("/normal-reward-upload/{reward_id}")
    async def upload_media(
        ...
        back_url = str(form.get("back_url") or f"/tower/redeem/{safe_id}")
        return RedirectResponse(url=back_url, status_code=303)

    @app.get("/normal-reward-media/{reward_id}")
    async def serve_media(reward_id: str):
        safe_id = _clean_reward_id(reward_id)
        info = _load_index()["rewards"].get(safe_id)
        if not info:
            raise HTTPException(status_code=404, detail="ยังไม่มีสื่อรางวัล")
