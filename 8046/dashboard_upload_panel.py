...
CONFIG_FILE = BASE_DIR / "dashboard_page_config.json"
...
def setup_dashboard_upload_panel(app) -> None:
    @app.middleware("http")
    async def dashboard_upload_panel_middleware(request: Request, call_next):
        path = request.url.path

        # เปิดรูปที่อัปโหลด
        if path.startswith("/tower/dashboard-media/"):
        ...
        # รับรูปจากแผงอัปโหลด
        if path == "/tower/dashboard-upload" and request.method == "POST":
            form = await request.form()

            image = form.get("dashboard_image")
            config = load_config()
        ...
        if path != "/tower/dashboard" or request.method != "GET":
            return response
        ...
        if image_name:
            hero_html = f"""
            <section class="dashboard-top-image">
              <img
                src="/tower/dashboard-media/{image_name}"
        ...
        <style>
          ...
          .dashboard-upload-panel label {{
            display: block;
            margin: 16px 0 7px;
          }}

          .dashboard-upload-panel input {{
            width: 100%;
            padding: 13px;
          ...
          .dashboard-upload-panel button[type="submit"] {{
            width: 100%;
            margin-top: 20px;
            padding: 14px;
        ...
        >
          ...
          <h2>จัดการรูป Dashboard</h2>
          <p>อัปโหลดภาพ และกำหนดลิงก์ของช่องในภาพ</p>

          <form
            method="post"
            action="/tower/dashboard-upload"
            enctype="multipart/form-data"
          >
            <label>อัปโหลดรูป Dashboard</label>
            <input
              type="file"
              name="dashboard_image"
              accept="image/*"
              onchange="previewDashboardImage(event)"
            >
        ...
        <script>
          function toggleInfiniUploadPanel() {{
            document
              .getElementById("dashboardUploadPanel")
