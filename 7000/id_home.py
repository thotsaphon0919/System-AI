from fastapi import FastAPI, UploadFile, File, Request, UploadFile, File
from fastapi.responses import HTMLResponse, JSONResponse, FileResponse, JSONResponse, FileResponse
...
<style id="INF_ID_REAL_UPLOAD_STYLE_V10">
  #creativeGatePage .gate-card{
...
function readFile(file, cb){
  ...
  // อัปโหลดเข้า backend จริง ไม่เก็บ base64 ใหญ่ใน localStorage แล้ว
  const form = new FormData();
  form.append("file", file);

  fetch("/api/id-home/upload", {
    method: "POST",
    body: form
  })
  .then(r => r.json())
  .then(data => {
    ...
    cb(url, data.type || file.type || "image");
  })
  .catch(err => {
    console.error(err);
    alert("อัปโหลดไม่สำเร็จ: backend รับไฟล์ไม่ได้");
  });
}
...
<!-- INF_ID_PERSIST_STATE_FIX_V11 -->
<style id="INF_ID_PERSIST_STATE_FIX_V11_STYLE">
  #idHomePage,
  #creativeGatePage{
...
<script id="INF_ID_PERSIST_STATE_FIX_V11">
(function(){
  if(window.__INF_ID_PERSIST_STATE_FIX_V11__) return;
  window.__INF_ID_PERSIST_STATE_FIX_V11__ = true;
  ...
  function getLocal(){
    return safeParse(localStorage.getItem(STORE));
  }
  ...
  function saveServer(d){
    fetch("/api/id-home/state", {
      method:"POST",
  ...
  // ทับ save เดิมให้บันทึกทั้ง localStorage + ไฟล์จริง
  window.save = function(d){
    saveLocal(d);
    saveServer(d);
  };

  // ทับ data เดิมให้อ่าน local state
  window.data = function(){
    return getLocal();
  };

  // ทับ readFile เดิมให้อัปโหลดเป็นไฟล์จริงใน backend
  window.readFile = function(file, cb){
    if(!file) return;
    ...
    .then(res=>{
      ...
      cb(res.url + "?t=" + Date.now(), res.type || file.type || "image");
    })
    .catch(err=>{
      console.error(err);
      alert("อัปโหลดไม่สำเร็จ");
  ...
  function applyState(d){
    ...
    if(d.idCover && window.setBox){
      setBox("idCoverBox","idCoverEmpty",d.idCover.src,d.idCover.type);
    }

    if(d.gate && window.setBox){
      setBox("gateCard","gateEmpty",d.gate.src,d.gate.type);
    }

    if(d.gallery && window.renderGallery){
      renderGallery(d.gallery || []);
    }
  }

  function loadServer(){
    fetch("/api/id-home/state?t=" + Date.now())
      .then(r=>r.json())
      .then(serverState=>{
        const localState = getLocal();

        // ถ้า server มีข้อมูล ใช้ server เป็นหลัก
        const hasServer = serverState && Object.keys(serverState).length > 0;
        const finalState = hasServer ? serverState : localState;
  ...
  // bind ปุ่ม cover ใหม่ กัน handler เก่าบางรอบไม่เซฟ
  function rebindUploads(){
    const coverInput = document.getElementById("idCoverInput");
    if(coverInput && !coverInput.dataset.persistV11){
      coverInput.dataset.persistV11 = "1";
      coverInput.addEventListener("change", function(e){
        const file = e.target.files && e.target.files[0];
        if(!file) return;

        readFile(file, function(src,type){
          const d = getLocal();
          d.idCover = {src,type};
          save(d);
          if(window.setBox) setBox("idCoverBox","idCoverEmpty",src,type);
        });
      });
    }

    const gateInput = document.getElementById("gateInput");
    if(gateInput && !gateInput.dataset.persistV11){
      gateInput.dataset.persistV11 = "1";
      gateInput.addEventListener("change", function(e){
        const file = e.target.files && e.target.files[0];
        if(!file) return;
        ...
        readFile(file, function(src,type){
          ...
          d.gate = {src,type};
          save(d);
          if(window.setBox) setBox("gateCard","gateEmpty",src,type);
        });
      });
    }

    const galleryInput = document.getElementById("galleryInput");
    if(galleryInput && !galleryInput.dataset.persistV11){
      galleryInput.dataset.persistV11 = "1";
      galleryInput.addEventListener("change", function(e){
        ...
        files.forEach(file=>{
          readFile(file, function(src,type){
            ...
            if(left === 0){
              save(d);
              if(window.renderGallery) renderGallery(d.gallery);
            }
          });
        });
      });
  ...
  setInterval(rebindUploads, 1500);
})();
</script>
...
def install_id_home(app: FastAPI):
    ...
    # INF_ID_PERSIST_BACKEND_V11
    @app.post("/api/id-home/upload")
    async def id_home_upload_v11(file: UploadFile = File(...)):
        upload_dir = Path("id_home_uploads")
        ...
        return JSONResponse({
            "ok": True,
            "url": f"/id-home-media/{name}",
            "type": content_type
        })

    @app.get("/id-home-media/{name}")
    async def id_home_media_v11(name: str):
        path = Path("id_home_uploads") / name
        if not path.exists():
            return JSONResponse({"ok": False, "error": "file not found"}, status_code=404)
        return FileResponse(path)

    @app.get("/api/id-home/state")
    async def id_home_get_state_v11():
        state_file = Path("id_home_state.json")
    ...
    @app.post("/api/id-home/state")
    async def id_home_save_state_v11(request: Request):
        state_file = Path("id_home_state.json")
        try:
    ...
    # INF_ID_REAL_UPLOAD_BACKEND_V10
    @app.post("/api/id-home/upload")
    async def id_home_real_upload(file: UploadFile = File(...)):
        upload_dir = Path("id_home_uploads")
        upload_dir.mkdir(exist_ok=True)
    ...
    @app.get("/id-home-media/{name}")
    async def id_home_media(name: str):
        path = Path("id_home_uploads") / name
        if not path.exists():
            return JSONResponse({"ok": False, "error": "file not found"}, status_code=404)
        return FileResponse(path)
