...
def install_detail_swipe_7000(app):
    ...
    db_file = data_dir / "detail_swipe_7000.json"
    remote_state_file = data_dir / "remote_show_state.json"
    lock = Lock()
...
.edBtn.danger{
 grid-column:1/-1;background:#2a0707;color:#ffb0a8;
 border-color:#a52d24
}
.hero{
...
function edShiftAfterDelete(deletedIndex){
 const next={};
 Object.entries(EDS).forEach(([key,value])=>{
...
<div class="edPanel" data-ed-panel="${index}" hidden>
 ...
 <div class="edActions">
  ...
  <button type="button" class="edBtn danger"
          data-ed-delete="${index}">ลบหน้านี้</button>
 </div>
</div>
        <section class="section">
          <div class="sectionHeadRow">
            ...
            <select class="zoneSelect" data-zone="${index}" aria-label="เลือกโซน">
              ...
              <option value="4" ${page.zone==='4' ? 'selected' : ''}>ZONE 4 · ครีเอทีฟ</option>
            </select>
          </div>
...
function bindEvents(){
 ...
 document.querySelectorAll('[data-ed-delete]').forEach(b=>{
  b.onclick=e=>{
   e.preventDefault();
   e.stopPropagation();
   deletePage(Number(b.dataset.edDelete||0));
  };
 });
...
async function deletePage(index){
  const page=group.pages[index];
  if(!page) return;
  if(!confirm(`ลบหน้า no.${page.no} ใช่ไหม\nรูป วิดีโอ รายละเอียด และแชทของหน้านี้จะถูกลบ`)) return;

  try{
    const result=await api(`/api/detail-page/${encodeURIComponent(page.no)}`,{method:'DELETE'});
    edShiftAfterDelete(index);

    if(result.empty){
      toast('ลบแล้ว ช่องกลับเป็นช่องว่าง');
      setTimeout(()=>{
    ...
    group=result.group;
    activeIndex=Math.max(0,Math.min(result.active_index||0,group.pages.length-1));
    render();
    setTimeout(()=>goTo(activeIndex,false),40);
    toast('ลบหน้านี้แล้ว');
  }catch(error){
    console.error(error);
...
(() => {
  ...
  document.addEventListener(
    ...
    @app.delete("/api/detail-page/{full_no}")
    def delete_detail_page(full_no: str):
        full_no, group_no, page_number = clean_full_no(full_no)

        with lock:
            data = load_db_unlocked()
            group = data.get("groups", {}).get(group_no)
            if not group or not isinstance(group.get("pages"), list):
                raise HTTPException(404, "ไม่พบหน้านี้")

            page_index = page_number - 1
            if page_index < 0 or page_index >= len(group["pages"]):
                raise HTTPException(404, "ไม่พบหน้านี้")

            removed = group["pages"].pop(page_index)

            # เลื่อนเลขหน้าที่เหลือให้ต่อกัน เช่น 5.1.3 กลายเป็น 5.1.2
            for index, page in enumerate(group["pages"], start=1):
                normalize_page(page, group_no, index)
            ...
            if group["pages"]:
                active_index = min(page_index, len(group["pages"]) - 1)
                result_group = public_group(group, active_index)
                first_page = group["pages"][0]
                sync_parent_thumbnail(
                    f"{group_no}.1",
                    first_page.get("media_url", ""),
                    first_page.get("media_type", ""),
                )
            else:
                # ไม่เหลือหน้าแล้ว ให้ช่องย่อยกลับเป็นช่องว่าง +
                data["groups"].pop(group_no, None)
                active_index = 0
                result_group = {
                    "group_no": group_no,
                    "active_index": 0,
                ...
                sync_parent_thumbnail(f"{group_no}.1", "", "")

            save_db_unlocked(data)

            # ลบไฟล์จริงเฉพาะไฟล์ที่อัปโหลดจากหน้ารายละเอียดนี้เอง
            # และไม่มีหน้าอื่นใช้อ้างอิงแล้ว เพื่อไม่กระทบรูปจากระบบเก่า
            removed_url = str(removed.get("media_url") or "")
            removed_name = Path(removed_url.split("?", 1)[0]).name
            if removed_name.startswith("detail_"):
                still_used = any(
                    str(page.get("media_url") or "") == removed_url
                    for item in data.get("groups", {}).values()
                    for page in item.get("pages", [])
                )
                if not still_used:
                    try:
                        (upload_dir / removed_name).unlink(missing_ok=True)
                    except Exception:
                        pass
            ...
            return JSONResponse(
                {
                    "ok": True,
                    "deleted_no": full_no,
                    "empty": not bool(result_group["pages"]),
