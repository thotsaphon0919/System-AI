...
def _secret_candidates(base: Path) -> list[str]:
    ...
    # The session cookie is signed once, centrally, using the secret at
    # <project_root>/8032/data/infini_session_secret.txt (see
    # id_entry_7000.py / id_hub_7000.py — same file, same secret, shared
    # across 7000/8032/8046 so one login works everywhere). This function
    # previously never checked that path at all, so on Render (base ==
    # the "7000" folder, no "data" sibling one level up) every lookup
    # here failed silently and every logged-in request fell back to
    # "guest" scope. Fixed to check the real shared location first.
