...
<style id="INF_AI_SHOP_DELETE_FIX_V1">
  .thumb{
  ...
  .delete-img{
    ...
    height:34px;
    border-radius:999px;
    border:1px solid rgba(255,120,0,.7);
...
<div class="page">
  ...
  <div class="section">
    <div class="card">
      ...
      <div class="clear-row">
        <button class="btn" onclick="clearImages()">ล้างรูปทั้งหมด</button>
        <button class="btn" onclick="clearResult()">ล้างผลลัพธ์</button>
      </div>

      <div class="poster-note">
        ตอนนี้เป็นระบบจัด layout จากรูปที่อัปโหลด ยังไม่ใช่ AI สร้างภาพใหม่จริง ถ้าจะให้รวมฉาก/จัดแสง/ทำเป็นโปสเตอร์ภาพเดียว ต้องต่อ API สร้างภาพภายหลัง
      </div>
    </div>
...
async function clearImages(){
  if(!confirm("ล้างรูปทั้งหมดใช่ไหม?")) return;
  STATE.images = [];
  await saveShop(false);
  renderUploads();
  clearResult();
...
function clearResult(){
  ...
  result.innerHTML = `<div class="empty">อัปโหลดรูป แล้วกด “AI จัดร้าน”</div>`;
}
...
def install_ai_shop_7000(app: FastAPI):
    ...
    # CRITICAL FIX: AI_SHOP_DIR / AI_SHOP_STATE used to be fixed relative
    # paths — the SAME single folder and SAME single state file for every
    # user on the server. One person's shop layout, uploaded product
    # photos, and everything else would silently overwrite everyone
    # else's. Every handler below now resolves a per-user path instead.
    ...
    async def ai_shop_upload(file: UploadFile = File(...)):
        ...
        try:
            ...
            if is_optimizable_image(file.filename or f"x{ext}"):
                stored_bytes, stored_ext, _ = optimize_image_bytes(raw, file.filename or f"x{ext}")
        except Exception:
            pass

        name = f"{uuid.uuid4().hex}{stored_ext}"
        out = scoped_upload_dir(BASE) / name
        out.write_bytes(stored_bytes)

        return JSONResponse({
            "ok": True,
            "url": scoped_upload_url(name),
            "type": content_type
        })

    @app.get("/api/ai-shop/state")
    async def ai_shop_get_state():
        state_file = scoped_data_file(BASE, "ai_shop_state.json", AI_SHOP_STATE)
